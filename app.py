...
APP_SUBTITLE = (
    "A noir chatbot for routing strange incidents through intake, validation, "
    "investigation, resolution, and closure."
)
LOG_PATH = Path(__file__).with_name("conversation_records.jsonl")
HERO_IMAGE_PATH = Path(
    __file__
).with_name("stranger_things___sketchers_of_will_by_godfathersky_dl703f8-pre.jpg")
STARTER_QUESTIONS = [
...
def init_state() -> None:
    ...
    if "active_theme_note" not in st.session_state:
        st.session_state.active_theme_note = (
            "Open a case, track the workflow, and keep a persistent log of every complaint."
        )
...
def render_starter_questions(user_label: str) -> None:
            ...
            if st.button(question, key=f"starter_question_{index}", use_container_width=True):
                ...
                with st.chat_message("assistant"):
                    with st.spinner("Routing complaint through the workflow..."):
                        result = run_case(question, user_label)
                    ...
                    with st.expander("Detailed output", expanded=False):
                        ...
                        st.write(
                            f"Workflow path: {' -> '.join(result.get('workflow_path', [])) or 'n/a'}"
                        )
                        st.write("Investigation notes:")
